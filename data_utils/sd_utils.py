...
g_class2id = {cls: id for id, cls in enumerate(g_classes)}
g_class2color = {
    'road': [0,255,0],
    'building': [0,0,255],
    'tree': [0,255,255],
...
def point_label_to_obj(input_filename, out_filename, label_color=True, easy_view=False):
    """ 
...
def street2blocks(data, label, num_point, street_block_size=1.0, stride=1.0,
    ...
    Args:
        ...
        label: N size uint8 numpy array
        num_point: int, how many points to sample in each block
        street_block_size: float, physical size of the block in meters
    ...
    """
    assert(stride<=street_block_size)
    ...
    if not random_sample:
        num_block_x = int(np.ceil((limit[0] - street_block_size) / stride)) + 1
        num_block_y = int(np.ceil(collect_point_label(limit[1] - street_block_size) / stride)) + 1
        for i in range(num_block_x):
            for j in range(num_block_y):
                xbeg_list.append(i*stride)
                ybeg_list.append(j*stride)
    else:
        num_block_x = int(np.ceil(limit[0] / street_block_size))
        num_block_y = int(np.ceil(limit[1] / street_block_size))
        if sample_num is None:
            sample_num = num_block_x * num_block_y * sample_aug
        for _ in range(sample_num):
            xbeg = np.random.uniform(-street_block_size, limit[0]) 
            ybeg = np.random.uniform(-street_block_size, limit[1]) 
    ...
    for idx in range(len(xbeg_list)): 
       xbeg = xbeg_list[idx]
       ybeg = ybeg_list[idx]
       xcond = (data[:,0]<=xbeg+street_block_size) & (data[:,0]>=xbeg)
       ycond = (data[:,1]<=ybeg+street_block_size) & (data[:,1]>=ybeg)
       cond = xcond & ycond
...
def street2blocks_plus(data_label, num_point, block_size, stride, random_sample, sample_num, sample_aug):
    """ 
        street2blocks with input filename and RGB preprocessing.
    """
    data = data_label[:,0:6]
    data[:,3:6] /= 255.0
    label = data_label[:,-1].astype(np.uint8)
    
    return street2blocks(data, label, num_point, block_size, stride, random_sample, sample_num, sample_aug)

def street2blocks_wrapper(data_label_filename, num_point, block_size=1.0, stride=1.0, random_sample=False, sample_num=None, sample_aug=1):
    if data_label_filename[-3:] == 'txt':
        ...
        exit()
    return street2blocks_plus(data_label, num_point, block_size, stride, random_sample, sample_num, sample_aug)

def street2blocks_plus_normalized(data_label, num_point, block_size, stride,
    ...
    data_batch, label_batch = street2blocks(data, label, num_point, block_size, stride,
                                          random_sample, sample_num, sample_aug)
...
def room2blocks_wrapper_normalized(data_label_filename, num_point, block_size=1.0, stride=1.0,
    ...
    if data_label_filename[-3:] == 'txt':
        ...
        print('Unknown file type! exiting.')
        exit()
    return street2blocks_plus_normalized(data_label, num_point, block_size, stride,
                                       random_sample, sample_num, sample_aug)
...
def room2samples_plus_normalized(data_label, num_point):
    """ 
        room2samples, with input filename and RGB preprocessing.
        for each block centralize XYZ, add normalized XYZ as 678 channels
    """
    data = data_label[:,0:6]
    data[:,3:6] /= 255.0
